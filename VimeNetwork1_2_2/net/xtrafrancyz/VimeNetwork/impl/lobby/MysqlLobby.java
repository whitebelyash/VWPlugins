...
Runnable {
    ...
    public MysqlLobby(VNPlugin plugin) {
        ...
        if (plugin.config.lobbyEnabled) {
            task = Bukkit.getScheduler().scheduleSyncRepeatingTask((Plugin)plugin, (Runnable)this, 5L, 30L);
    ...
    public void setMenuText(String ... lines) {
        this.menuInfo = lines == null || lines.length == 0 ? "NULL" : "'" + StringEscapeUtils.escapeSql((String)Joiner.on((String)"^").join((Object[])lines)) + "'";
    }
