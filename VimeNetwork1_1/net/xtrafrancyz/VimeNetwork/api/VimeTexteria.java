...
public class VimeTexteria {
   ...
   public void showInsufficientlyCoins(Player player) {
      Texteria2D.add(this.genInvErrorMessage(5000L, "&cУ вас недостаточно коинов", "Их можно заработать, либо же купить в ЛК", "&ohttp://cp.vimeworld.ru").setOnClick(new OnClick(Action.URL, "http://cp.vimeworld.ru/exchange")), new Player[]{player});
   }
   ...
   private Text genInvErrorMessage(long duration, String... message) {
      return (Text)((Text)((Text)((Text)((Text)(new Text("vn.api.topmsg", message)).setPosition(Position.TOP)).setOffset(0, 30)).setDuration(duration)).setVisibility(new Visibility.Always())).setScale(2.0F);
   }
   ...
   public void showCountdown(int seconds, String message, Supplier players) {
      for(int i = 0; i <= seconds; ++i) {
         Bukkit.getScheduler().scheduleSyncDelayedTask(VNPlugin.instance(), () -> {
            ...
            Text text = (Text)((Text)((Text)((Text)((Text)((Text)(new Text((String)null, new String[]{String.valueOf(seconds - i)})).setScale(6.0F)).setAnimation((new Animation2D()).setFinish((new Animation2D.Params()).setScale(25.0F)).setStart((new Animation2D.Params()).setScale(-6.0F)))).setColor(color)).setFadeStart(400)).setFadeFinish(500)).setDuration(1000L);
            if (seconds == i) {
