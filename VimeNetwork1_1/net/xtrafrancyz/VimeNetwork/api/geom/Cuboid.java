...
public class Cuboid implements Iterable {
   ...
   public void expand(Vec3i change) {
      if (change.x > 0) {
         this.max = this.max.setX(this.max.x + change.x);
      } else if (change.x < 0) {
         this.min = this.min.setX(this.min.x + change.x);
      }

      if (change.y > 0) {
         this.max = this.max.setY(this.max.y + change.y);
      } else if (change.y < 0) {
         this.min = this.min.setY(this.min.y + change.y);
      }

      if (change.z > 0) {
         this.max = this.max.setZ(this.max.z + change.z);
      } else if (change.z < 0) {
         this.min = this.min.setZ(this.min.z + change.z);
      }
