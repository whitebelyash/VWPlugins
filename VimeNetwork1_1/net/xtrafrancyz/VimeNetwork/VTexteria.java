...
public class VTexteria {
   ...
   private static final Text TC_SERVER_ID;
   private static final ByteMap CALLBACK_OPEN_GOALS;
   private static final ByteMap CALLBACK_OPEN_STREAMS;
   ...
   public static void showUsername(VPlayer player) {
      ...
      Texteria2D.add(((Text)((Text)(new Text("vn.n", new String[]{text})).setPosition(Position.TOP_LEFT)).setOffset(2, 2)).setVisibility(new Visibility.IngameNotF3()), new Player[]{player.player});
   }
   ...
   public static void showCoins(VPlayer player) {
      ...
      if (player.getMultipliers().getCurrentMultiplier() != 1) {
         ...
         if (prev != player.coinsTexteria) {
            Texteria2D.add(((Text)((Text)x.setPosition(Position.BOTTOM_LEFT)).setOffset(2, 16)).setVisibility(new Visibility.Always()), new Player[]{player.player});
         }
      } else {
         elem = (Text)((Text)(new Text("vn.c.text", new String[]{text})).setOffset(2, 16)).setPosition(Position.BOTTOM_LEFT);
      }

      Texteria2D.add(elem.setVisibility(new Visibility.Always()), new Player[]{player.player});
   }
   ...
   public static void showAchievementMessage(Achievement achievement, Player player) {
      ...
      Bukkit.getScheduler().scheduleSyncDelayedTask(VNPlugin.instance(), () -> Texteria2D.add(((Text)((Text)((Text)((Text)(new Text("a3", new String[]{"&a✔"})).setAttachment(new Attachment("a1", Position.RIGHT))).setDuration(4250L)).setScale(4.0F)).setOffset(15, 0)).setFadeStart(400), new Player[]{player}), 15L);
   }
   ...
   public static void showCoinsChange(VPlayer player, int amount) {
      if (amount > 0) {
         Texteria2D.add(((Text)((Text)((Text)((Text)((Text)((Text)(new Text("vn.c.change", new String[]{"+" + amount})).setColor(-16711936)).setOffset(1, 0)).setFadeFinish(500)).setDuration(1200L)).setVisibility(new Visibility.Always())).setAnimation(ANIM_ADD_COINS)).setAttachment((new Attachment("vn.c.text", Position.RIGHT)).setRemoveWhenParentRemove(false)), new Player[]{player.player});
      } else if (amount < 0) {
   ...
   public static void showGoalMessage(NetworkPlayer player) {
      goalMessage(player.getBukkitPlayer(), IntColor.setAlpha(-16537100, 200), "&lАктивно заданий: " + player.getGoals().getActiveGoals().size(), "Нажмите для просмотра");
   }
   ...
   static {
      TC_SERVER_ID = (Text)((Text)((Text)(new Text("vn.s", new String[]{VimeNetwork.lobby().getServerId()})).setPosition(Position.TOP_LEFT)).setOffset(2, 12)).setVisibility(new Visibility.IngameNotF3());
      CALLBACK_OPEN_GOALS = new ByteMap();
      CALLBACK_OPEN_STREAMS = new ByteMap();
